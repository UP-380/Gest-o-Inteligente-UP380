...
/**
 * Modal reutilizável para criar/editar adquirente
 */
const AdquirenteModal = ({
  isOpen,
  onClose,
  onSubmit,
  formData,
          ...
          <form onSubmit={handleSubmit} className="adquirente-form">
            <div className="form-group">
              <label className="form-label" style={{ marginBottom: '8px', display: 'block', fontWeight: '500' }}>
                Nome do Adquirente <span className="required" style={{ color: '#e74c3c' }}>*</span>
              </label>
              <input
                ref={nomeInputRef}
                type="text"
                ...
                maxLength={100}
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && formData.nome?.trim() && !submitting) {
                    handleSubmit(e);
                  }
                }}
              />
...
export default AdquirenteModal;
