...
const RelatorioTempo = () => {
    ...
    const [selectedResponsavel, setSelectedResponsavel] = useState('all'); // 'all' or usuario_id
    ...
    const fetchInitialData = async () => {
        ...
        try {
            ...
            if (selectedResponsavel !== 'all') {
                const u = usuarios.find(u => u.id === selectedResponsavel);
                if (u) targetUserIds = [u.id];
            } else {
                targetUserIds = usuarios.map(u => u.id);
            }
    ...
    }, [dataInicio, dataFim, selectedResponsavel, usuarios]);
        ...
        <Layout>
            <div className="container">
                <main className="main-content relatorio-tempo-main">
                    ...
                    {/* Toolbar */}
                    <div className="relatorio-toolbar" ref={popoverRef}>
                        {/* Periodo */}
                        <div className="toolbar-item">
                            <div
                                className={`toolbar-icon ${activePopover === 'periodo' ? 'active' : ''}`}
                                onClick={() => setActivePopover(activePopover === 'periodo' ? null : 'periodo')}
                                title="Filtrar Período"
                            >
                                <i className="fas fa-calendar-alt"></i>
                            </div>
                            {activePopover === 'periodo' && (
                                <div className="toolbar-popover period-popover" style={{ minWidth: '350px' }}>
                                    <div style={{ padding: '0.5rem 0', fontWeight: '600', color: '#1e293b', marginBottom: '1rem', borderBottom: '1px solid #f1f5f9' }}>
                                        Selecione o Período
                                    </div>
                                    <FilterPeriodo
                                        ...
                                        onClose={() => setActivePopover(null)} // Keeps existing behavior
                                    />
                                </div>
                            )}
                        </div>
                        ...
                        <div className="toolbar-item">
                            <div
                                className={`toolbar-icon ${activePopover === 'responsavel' ? 'active' : ''}`}
                                onClick={() => setActivePopover(activePopover === 'responsavel' ? null : 'responsavel')}
                                title="Filtrar Responsável"
                            >
                            ...
                            {activePopover === 'responsavel' && (
                                <div className="responsavel-card-dropdown">
                                    ...
                                    <div className="responsavel-card-dropdown-content custom-scrollbar">
                                        <div
                                            className={`responsavel-card-option ${selectedResponsavel === 'all' ? 'selected' : ''}`}
                                            onClick={() => { setSelectedResponsavel('all'); setActivePopover(null); }}
                                        >
                                        ...
                                        {filteredUsuariosSearch.map(u => (
                                            <div
                                                key={u.id}
                                                className={`responsavel-card-option ${selectedResponsavel === u.id ? 'selected' : ''}`}
                                                onClick={() => { setSelectedResponsavel(u.id); setActivePopover(null); }}
                                            >
                                                <Avatar
