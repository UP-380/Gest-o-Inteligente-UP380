...
const DelegarTarefas = () => {
  ...
  // Cache persistente para tempo estimado total
  // Estrutura: { [responsavelId]: { [periodoKey]: tempoEmMs } }
  const tempoEstimadoCacheRef = useRef({});
  ...
  const limparCacheHorasContratadas = useCallback(() => {
    ...
    tempoEstimadoCacheRef.current = {};
    console.log('🧹 [CACHE] Caches limpos (horas contratadas e tempo estimado)');
  }, []);
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.success && result.data) {
          ...
          if (!temPeriodo) {
            // Sem filtro de período (apenas paginação simples), usar regras diretas
            registrosCalculados = regras;
          } else {
              ...
              if (periodoInicio <= periodoFim) {
                ...
                datasValidas.forEach(dataStr => {
                  ...
                  registrosCalculados.push({
                    ...
                    responsavel_nome: regra.responsavel_nome,
                    cliente_nome: regra.cliente_nome,
                    produto_nome: regra.produto_nome,
                    tarefa_nome: regra.tarefa_nome,
          ...
          if (periodoAUsar.inicio && periodoAUsar.fim) {
            ...
            // OTIMIZAÇÃO: Busca incremental de tempo estimado total com CACHE
            const responsaveisAtuais = valoresAUsar.responsavel
              ? (Array.isArray(valoresAUsar.responsavel) ? valoresAUsar.responsavel : [valoresAUsar.responsavel])
              : [];

            const periodoKey = `${periodoAUsar.inicio}_${periodoAUsar.fim}`;
            const idsParaBuscar = new Set();
            const dadosEmCache = {};

            // 1. Identificar quem já está no cache e quem precisa buscar
            if (responsaveisAtuais.length > 0) {
              responsaveisAtuais.forEach(id => {
                const idStr = String(id).trim();
                // Verifica se temos no cache para este período E com os mesmos filtros adicionais (simplificado: cache por responsável+período por enquanto)
                // Idealmente o cache deveria considerar filtros adicionais, mas vamos assumir que responsável+período é a chave principal
                if (tempoEstimadoCacheRef.current[idStr] && tempoEstimadoCacheRef.current[idStr][periodoKey] !== undefined) {
                  dadosEmCache[idStr] = tempoEstimadoCacheRef.current[idStr][periodoKey];
                } else {
                  idsParaBuscar.add(idStr);
                }
              });
            } else {
              // Se não há filtro de responsável, busca tudo (fallback para comportamento padrão sem cache granular por enquanto)
